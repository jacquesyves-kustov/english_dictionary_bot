...
class RabbitClient:
    ...
    def publish(self, message_body: str, queue_name: str):
        ...
        print(" [WORD] Message", message_body, "is sent!")

    def consume(self, handler, queue_name: str):
        """
        Общий интерфейс для отправки сообщений

        handler - функция для обработки сообщений (описаны в .message_processing.py)
        queue_name - очередь брокера
        """

        def callback(ch, method: pika.spec.Basic.Deliver, properties: pika.spec.BasicProperties, body):
            message_body = body.decode()
            print(" [DEF] Received message:", message_body)
        ...
        # Безопасное завершение работы для тестов
        try:
